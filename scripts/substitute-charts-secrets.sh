...
for environment in "${environments[@]}"; do
  ...
  for cluster in ${CLOUDSDK_CONTAINER_CLUSTERS}; do
    ...
    sed -i "s/pwd: /pwd: ${!FORECAST_POSTGRES_DB_PASSWORD}/" "./charts/dealsheet-api/values-${cluster}.yaml"
    sed -i "s/pwd: /pwd: ${!FORECAST_POSTGRES_DB_PASSWORD}/" "./charts/calendar-api/values-${cluster}.yaml"
